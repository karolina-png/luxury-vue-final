<template>
  <div id="container-articles">
            ...
            <form id="formnew" enctype="multipart/form-data">
              ...
              <section class="photo-container">
                <div class="photo-prev">
                  <input type="file" id="new-article-input" @change="show_image" style="display: none" />
                  <!-- Image client exist and is not loading a new image -->
                  <div class="preview" v-if="articles.url && !loading">
                    <span class="material-symbols-outlined clear-image" @click="clear_image('new-article-input')">
                      close
                    </span>
                    <img @click="open_browser('new-article-input')" :src="articles.url" />
                  </div>
                  <!-- Image client not exist and is not loading a new image -->
                  <span v-if="!articles.url && !loading" class="material-symbols-outlined"
                    @click="open_browser('new-article-input')">
                    account_circle
                  </span>

                  <div v-if="loading" class="loading" @click="open_browser('new-article-input')"></div>
    ...
    <div class="modal fade" id="editArticle" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
      aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body formnew">
            <form id="formnew" enctype="multipart/form-data">
              ...
              <div class="mb-2">
                ...
                <input type="file" class="form-control" id="image" @change="get_image">
              </div>
              <figure>
                <img v-if="imagenminiatura !== null" :src="imagen" width="50%" alt="">
                <img v-if="imagenminiatura == null" :src="articles_edit.image" width="50%" height="20%" alt="">
              </figure>
          ...
          <div class="modal-footer">
            <button type="button" class="text-black " data-bs-dismiss="modal" @click="update_article()">Edit</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async new_article() {
      ...
      let response = await this.axios.post("/api/articles/", formData)
        .then(response => {
    ...
    async update() {
      ...
      console.log("Id: " + id);
      await this.axios.put("/api/articles/" + id, formData_edit)
        .then(response => {
